...
class DltsEpub extends Epub {
    constructor( explodedEpubDir ) {
        ...
        let dcCreatorElements = this.package._xml.metadata[ 'dc:identifier' ];
        let packageUniqueIdentifierAttribute = this.package._xml[ 'unique-identifier' ];
        if ( ! Array.isArray( dcCreatorElements ) ) {
            dcCreatorElements = [ dcCreatorElements ];
        }

        this.dlts = {
            metadata : {
                identifier : dcCreatorElements.find( ( element ) => {
