...
class Epub {
    constructor( explodedEpubDir ) {
        ...
        // See https://jira.nyu.edu/browse/NYUP-857?focusedCommentId=1862161&page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel#comment-1862161.
        const massagedXml = XML.parse(
            fs.readFileSync( packageFilePath, 'utf8' )
        );
        if ( 'opf:metadata' in massagedXml ) {
            massagedXml.metadata = massagedXml[ 'opf:metadata' ];
            delete massagedXml[ 'opf:metadata' ];
        }
        this.package = {
