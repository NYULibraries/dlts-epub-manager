...
let syncFetch = require( 'sync-fetch' );
...
function getAuthorSortKey( authorList ) {
    ...
    let authors = authorList.split( /\s*,\s*/ );
    ...
    let pieces = firstAuthorInList.match( /(.*)\s+(.+)$/ );

    let presumedGivenName = pieces[ 1 ];
    let presumedSurname   = pieces[ 2 ];
...
function getLicenseData( licenseName ) {
    const licenseData = LICENSE_TABLE[ licenseTable ];

    if ( LICENSE_TABLE[ licenseName ] ) {
        return licenseData;
    } else {
...
function getMetadataDir( em ) {
    ...
    let metadataRepo             = em.conf.metadataRepo;
    let metadataRepoBranch       = em.conf.metadataRepoBranch;
    let metadataRepoSubdirectory = em.conf.metadataRepoSubdirectory;
