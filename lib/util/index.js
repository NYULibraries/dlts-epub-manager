...
// Quick and dirty temp function to mimic what legacy script does when determining
// which files contain references to cover.html that need to be changed.
// Eventually there will be library code that parses all EPUB metadata and perhaps
// also provides an abstraction for the EPUB data in general.
function tempGetManifestItemsFilePathsFromEpubPackageFile( epubDir ) {
    let packageFile          = tempGetPackageFilePath( epubDir );
...
function tempGetPackageFilePath( epubDir ) {
    let containerFileXmlString = fs.readFileSync( `${epubDir}/META-INF/container.xml` );
    let containerXml           = XML.parse( containerFileXmlString );
    let packageFile            = `${epubDir}/${containerXml.rootfiles.rootfile[ 'full-path' ]}`;
...
module.exports = {
    ...
    jsonStableStringify,
    tempGetManifestItemsFilePathsFromEpubPackageFile,
    tempGetPackageFilePath,
};
