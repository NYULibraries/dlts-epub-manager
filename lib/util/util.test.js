...
describe( 'util', () => {
    ...
    const LICENSES = {
        ...
        'https://creativecommons.org/licenses/by-nc-sa/4.0/' : 'Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International License',
    ...
    describe( '#getAuthorSortKey', () => {
        it( 'should return correct author sort keys', () => {
            Object.keys( AUTHORS ).forEach( originalAuthors => {
                const expectedAuthors = AUTHORS[ originalAuthors ];
                const got             = util.getAuthorSortKey( originalAuthors );

                expect( got ).toEqual( expectedAuthors );
            } );
        } );
    } );

    describe( '#licenseData', () => {
        it( 'should return correct license data for link', () => {
            Object.keys( LICENSES ).forEach( licenseLink => {
                const expectedLicenseName = LICENSES[ licenseLink ];
                const got                 = util.getLicenseData( licenseLink ).name;

                expect( got ).toEqual( expectedLicenseName );
            } );
        } );
    } );
    ...
    describe( '#getTitleSortKey', () => {
        it( 'should return correct title sort keys', () => {
            Object.keys( TITLES ).forEach( originalTitle => {
                const expectedTitle = TITLES[ originalTitle ];
                const got           = util.getTitleSortKey( originalTitle );

                expect( got ).toEqual( expectedTitle );
            } );
        } );
    } );
    ...
    describe( '#normalizeDltsEpubIdentifier', () => {
        ...
        it( 'should correctly normalized "close-enough" identifiers', () => {
            const input    = 'urn:isbn:9780814739617';
            const expected = '9780814739617';

            const got = util.normalizeDltsEpubIdentifier( input );
