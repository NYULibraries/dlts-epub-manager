...
* Intake of EPUB files and optional publisher-provided metadata: creation of the normalized,
exploded EPUBs that are stored in the `nyu-press-readium-epub-content private repo`,
and creation of the normalized metadata files that are used for publication and
that are stored in [dlts-epub-metadata](https://github.com/NYULibraries/dlts-epub-metadata).
If publisher-provided metadata is not provided the normalized metadata files will
not be created. 
* Solr indexing of EPUB metadata from the source files generated by intake (or
similar files generated through an outside method)
* Management of the EPUB handles in the handle server
* Creation and editing of [epub_library.json files](https://github.com/readium/readium-js-viewer/blob/master/epub_content/epub_library.json)
, which are the files used by
...
* Node.js version 4.x or higher (for ES6 support)
...
# files.
./em intake add local
# Do not create metadata files -- perhaps because publisher did not provide
# metadata files and the normalized metadata files have already been created
# via an outside process.
./em intake add --skip-metadata local
```
...
somebody@host:~/epub-manager$ ./em help
  ...
  Commands:
    ...
    intake add [options] [configuration]       Intake EPUBs and generate Readium versions and associated metadata
                                               files.
    ...
    verify                                     Verify integrity of published collection, handles, and metadata
                                               indexes.
...
are executed using specific commands: `handles`, `intake`, `load`, `readium-json`,
and `solr`.
...
# Unit tests - by module or class
mocha test/unit/DltsEpub
mocha test/unit/DltsOnix
mocha test/unit/util
...
* **intakeEpubDir**: directory containing the *.epub files and EPUB metadata to
be processed by the intake system 
