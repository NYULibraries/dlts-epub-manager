...
`em` is a command-line program for managing the NYU Press open access EPUBs made available online
 on [Open Square](http://opensquare.nyupress.org/).

## Overview

Current functions:

* Intake of EPUB files: creation of the normalized, exploded EPUBs and book cover
thumbnail images that are stored in the `nyu-press-readium-epub-content private repo`.
* Solr indexing of EPUB metadata from the source files generated by the `metadata`
command
* Management of the EPUB handles in the handle server
* Creation of the normalized metadata files that are used for publication that
are stored in [dlts-epub-metadata](https://github.com/NYULibraries/dlts-epub-metadata)
* Creation and editing of [epub_library.json files](https://github.com/readium/readium-js-viewer/blob/master/epub_content/epub_library.json), which are the files used by
[ReadiumJS viewer](https://github.com/readium/readium-js-viewer) to describe a library
(note that `epub_library.json` is a legacy format file that has been superseded by
`epub_library.opds` -- OPDS XML)
...
files in `config/`. 
The usernames and passwords for our restful handle servers and for the Supafolio API
 need to be specified in these files:
...
somebody@host:~/epub-manager$ cat config-private/dev.json 
{
    ...
    "supafolioApiKey" : "[SAUPAFOLIO API KEY]"
}
...
**Step 4)** Make a local configuration if needed.  The `intake` and `metadata`
commands currently require a local configuration file (see
[Special note about configuration of `intake`](#special-note-about-configuration-of-intake)
and
[Special note about configuration of `metadata`](#special-note-about-configuration-of-metadata)
):
...
See [Configuration file format](#configuration-file) for more details.  Also
see [Special note about configuration of `intake`](#special-note-about-configuration-of-intake).
...
```
# Intake EPUB files and output normalized exploded EPUB directories and metadata
# files.
./em intake add local
```
...
`em` is built using [Vorpal](https://github.com/dthree/vorpal), a Node.js framework
for building interactive CLI applications.  The various EPUB management functions
are executed using specific commands: `handles`, `intake`, `load`, `metadata`, `readium-json`,
and `solr`.
...
detailed use cases.  No detailed use case is provided for `intake` because this
command has only one subcommand `add` and is usually run as a one-shot
(see
[Special note about configuration of `intake`](#special-note-about-configuration-of-intake)).
...
`intake` configuration is not included in the
[dev](https://github.com/NYULibraries/dlts-epub-manager/blob/master/config/dev.json),
[stage](https://github.com/NYULibraries/dlts-epub-manager/blob/master/config/stage.json),
and [prod](https://github.com/NYULibraries/dlts-epub-manager/blob/master/config/prod.json)
configurations because `intake` is a one-shot process that is done when a collection
is first received by the publisher, and is never run again as part of a publication job
to dev, stage, prod.  `intake` creates the data and metadata that are consumed
by the other commands like `handles` or `solr` that are run on a more regular basis
against dev, stage, and prod servers.

### Special note about configuration of `metadata`

`metadata` configuration is not included in the
[dev](https://github.com/NYULibraries/dlts-epub-manager/blob/master/config/dev.json),
[stage](https://github.com/NYULibraries/dlts-epub-manager/blob/master/config/stage.json),
and [prod](https://github.com/NYULibraries/dlts-epub-manager/blob/master/config/prod.json)
configurations because `metadata` targets a local directory and does not run against
dev, stage, prod servers or in dev, stage, and prod environments.  This local directory
will usually be a local clone of [dlts-epub-metadata](https://github.com/NYULibraries/dlts-epub-metadata),
with dev, stage, or prod branches checked out as needed.

The `metadata` currently determines which ISBNs to fetch metadata for by reading the
subdirectory names in `intakeEpubDir` or the ISBN list in `intakeEpubList`. 

##Future enhancements

* Solr indexing of EPUB full-text content
* Creation and editing of [epub_library.opds files](https://github.com/readium/readium-js-viewer/blob/master/epub_content/epub_library.json)
